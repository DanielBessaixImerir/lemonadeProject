...
@app.route('/')
def homePage():
	return "Bonjour je suis une page de garde. Connectez vous à l'adresse /static/index.html"
...
def createPlayer():
	...
	player = {"name":"", "location":{"longitude":"", "latitude":""}}
	player['location']['latitude'] = random.randint(0, 475)
	player['location']['longitude'] = random.randint(0, 730)
	player['name'] = name
	randy = player['location']['latitude']
	randx = player['location']['longitude']
	strRandX = str(randx)
	strRandY = str(randy)
...
@app.route('/players/<string:playername>', methods=['DELETE'])
def deletePlayer(playername):
	db = Db()

	db.execute("DELETE FROM player WHERE player_name="+ playername +";")
	
	db.close()

	return json.dumps("Delete successfully"), 200, {'Content-Type': 'application/json'}
...
def resetGame():
	...
	db.execute(""" 
					...
					UPDATE current_day SET current_weather='sunny' WHERE day_name='today';
					UPDATE current_day SET current_forecast='sunny' WHERE day_name='today';
	""")
