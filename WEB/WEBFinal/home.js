...
//	prices();
	...
	$("#toggleChat").on('click', toggleChat);
	$("#deletePlayer").on('click', deletePlayer);
	$("#toggleInformations").on('click', toggleInformations);
...
function toggleInformations(){
	if(toggled2){
		$('#toggleInformationsBox').fadeIn(500);
		toggled2 = false;
	}else{
		$('#toggleInformationsBox').fadeOut(500);
		toggled2 = true;
	}
}
...
function setup(){
	...
	$('#chatBox').fadeOut(1);
	$('#toggleInformationsBox').fadeOut(1);
		...
		.done(function(data) {
			...
			for (var items in itemList) {
				for(var i = 0; i<itemList[items].length; i++){
					...
					var owner = itemList[items][i]['owner'];
					ctx.beginPath();
					ctx.lineWidth = 1;
					var writeStyle = "";
					if(String(owner).valueOf() == String(currentPlayer).valueOf()){
					...
					ctx.fillStyle=writeStyle;
					ctx.fillText(owner, latitude, longitude);
					ctx.fillText(owner, latitude, longitude);
					ctx.stroke();
				}
			}
...
function deletePlayer(){
	var currentPlayer = document.getElementById("currentPlayer").innerHTML;
	var deleteUrl = 'https://tranquil-reef-75630.herokuapp.com/players/' + currentPlayer;
